...
namespace DFC.Api.Lmi.Import.Services
{
    public class LmiWebhookReceiverService : ILmiWebhookReceiverService
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(apiEndpoint) && apiEndpoint.EndsWith($"/{Constants.ApiForJobGroups}", StringComparison.OrdinalIgnoreCase))
            {
                return MessageContentType.Publish;
            }
